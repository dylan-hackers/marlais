...
install_syntax_entry (char *name, syntax_fun fun)
{
    ...
    h = ((marlais_int_t) sym) % SYNTAX_TABLE_SIZE;
    entry = MARLAIS_MALLOC_GENERAL (struct syntax_entry);
...
define_generic_function_eval (Object form)
{
  ...
  gf = marlais_make_generic (name, params, MARLAIS_NIL);
    ...
    while (marlais_is_pair_p (clauses)) {
      ...
      if (marlais_is_pair_p (clause)) {
        if (CAR (clause) == use_symbol) {
          ...
          Object renames = MARLAIS_NIL;
          Object exports = MARLAIS_NIL;
...
for_eval (Object form)
{
  ...
  clause_types = vars = inits = MARLAIS_NIL;
                    ...
                    Object *inits_ptr)
{
  ...
  while (marlais_is_pair_p (var_forms)) {
    ...
    if (marlais_is_pair_p (var_spec) || marlais_is_name_p (var_spec)) {
      ...
      init = marlais_make_list (start, by, negative, termination, bound, NULL);
      ...
      var = get_variable (SECOND (var_form));
      init = marlais_make_list (MARLAIS_NIL,
                     marlais_eval (THIRD (var_form)),
                     MARLAIS_NIL,
                     NULL);
    }
    *clause_types_ptr = marlais_cons (clause_type, MARLAIS_NIL);
    *vars_ptr = marlais_cons (var, MARLAIS_NIL);
    *inits_ptr = marlais_cons (init, MARLAIS_NIL);
    ...
    if (clause_type == collection_keyword) {
      ...
      if (termination == MARLAIS_FALSE) {
        ...
        if (negative == MARLAIS_TRUE) {
          if (MARLAIS_TRUE == marlais_eval (marlais_make_list (lesser_symbol,
                                            current,
                                            bound,
                                            NULL))) {
            return 1;
          }
        } else if (MARLAIS_TRUE == marlais_eval (marlais_make_list (greater_symbol,
                                                 current,
                                                 bound,
                                                 NULL))) {
          return 1;
        ...
        if (MARLAIS_TRUE == marlais_eval (marlais_make_list (lesser_equal_symbol,
                                          current,
                                          bound,
                                          NULL))) {
          return 1;
        ...
        if (MARLAIS_TRUE == marlais_eval (marlais_make_list (greater_equal_symbol,
                                          current,
                                          bound,
                                          NULL))) {
          return 1;
                                     ...
                                     Object inits)
{
  ...
  while (marlais_is_pair_p (clause_types)) {
    ...
    if (clause_type == variable_keyword) {
       ...
       *  (+ var increment)
       */
      new_value = marlais_eval (marlais_make_list (plus_symbol,
    ...
    *new_values_ptr = marlais_cons (new_value, MARLAIS_NIL);
